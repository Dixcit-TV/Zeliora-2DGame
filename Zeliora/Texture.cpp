...
Texture::Texture( const std::string& text, TTF_Font *pFont, const Color4f& textColor, bool multiLine)
	:m_Id{}
	,m_Width{ 10.0f }
	,m_Height{ 10.0f }
	,m_CreationOk{ false }
{
	CreateFromString( text, pFont, textColor, multiLine);
}

Texture::Texture( const std::string& text, const std::string& fontPath, int ptSize, const Color4f& textColor, bool multiLine)
	:m_Id{}
	,m_Width{ 10.0f }
	,m_Height{ 10.0f }
	,m_CreationOk{ false }
{
	CreateFromString( text, fontPath, ptSize, textColor, multiLine);
}
...
void Texture::CreateFromString( const std::string& text, const std::string& fontPath, int ptSize, const Color4f& textColor, bool multiLine)
{
	m_CreationOk = true;
...
void Texture::CreateFromString( const std::string& text, TTF_Font *pFont, const Color4f& color, bool multiLine)
{
	m_CreationOk = true;
	if ( pFont == nullptr )
	{
	...
	SDL_Surface* pLoadedSurface = nullptr;
	...
	case true:
		{
			...
			while (!ssText.eof())
			{
				std::getline(ssText, line, '\n');
				TTF_SizeText(pFont, line.c_str(), &w, &h);
				if (w > maxWidth)
				{
					maxWidth = w;
				}
			...
			pLoadedSurface = TTF_RenderText_Blended_Wrapped(pFont, text.c_str(), textColor, maxWidth);
		}
		break;
	case false:
		pLoadedSurface = TTF_RenderText_Blended(pFont, text.c_str(), textColor);
		break;
	}
