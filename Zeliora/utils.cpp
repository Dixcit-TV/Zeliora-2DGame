...
void utils::DrawRect(const Rectp& rect, float lineWidth)
{
	DrawRect(rect.center.x - rect.halfWidth, rect.center.y - rect.halfHeight, rect.halfWidth * 2, rect.halfHeight * 2, lineWidth);
}

void utils::FillRect(const Rectp& rect)
{
	FillRect(rect.center.x - rect.halfWidth, rect.center.y - rect.halfHeight, rect.halfWidth + rect.halfWidth, rect.halfHeight + rect.halfHeight);
}
...
bool utils::IsPointInRect(const Point2f& p, const Rectp& r)
{
	return (p.x >= r.center.x - r.halfWidth &&
		p.x <= r.center.x + r.halfWidth &&
		p.y >= r.center.y - r.halfHeight &&
		p.y <= r.center.y + r.halfHeight);
}
...
bool utils::IsOverlapping(const Rectp& r1, const Rectp& r2)
{
	if ((r1.center.x + r1.halfWidth) <= (r2.center.x - r2.halfWidth) || (r2.center.x + r2.halfWidth) <= (r1.center.x - r1.halfWidth))
	{
		return false;
	}

	if ((r1.center.y - r1.halfHeight) >= (r2.center.y + r2.halfHeight) || (r2.center.y - r2.halfHeight) >= (r1.center.y + r1.halfHeight))
	{
		return false;
	}
...
bool utils::IsOverlapping(const Rectp& r, const Circlef& c)
{
	// Is center of circle in the rectangle?
	if (IsPointInRect(c.center, r))
	{
	...
	// Check line segments
	if (utils::DistPointLineSegment(c.center, Point2f{ r.center.x - r.halfWidth, r.center.y - r.halfHeight }, Point2f{ r.center.x - r.halfWidth, r.center.y + r.halfHeight }) <= c.radius)
	{
		return true;
	}
	if (utils::DistPointLineSegment(c.center, Point2f{ r.center.x - r.halfWidth, r.center.y - r.halfHeight }, Point2f{ r.center.x + r.halfWidth, r.center.y - r.halfHeight }) <= c.radius)
...
bool utils::IntersectRectLine(const Rectp& r, const Point2f& p1, const Point2f& p2, float& intersectMin, float& intersectMax)
{
	float x1{ (r.center.x - r.halfWidth - p1.x) / (p2.x - p1.x) };
	float x2{ (r.center.x + r.halfWidth - p1.x) / (p2.x - p1.x) };
	float y1{ (r.center.y - r.halfHeight - p1.y) / (p2.y - p1.y) };
	float y2{ (r.center.y + r.halfHeight - p1.y) / (p2.y - p1.y) };
...
float utils::GetRand(float max, float min)
{
	return float(rand()) / RAND_MAX * (max - min) + min;
}

int utils::GetRand(int max, int min)
{
	return rand() % (max - min + 1) + min;
}

int utils::RoundToClosest(float number, int multiple)
{
	int res = int(number + multiple / 2.f);
	return res -= res % multiple;
}

float utils::GetLength2(const Point2f& p0, const Point2f& p1)
{
	return float(pow(p1.x - p0.x, 2)) + float(pow(p1.y - p0.y, 2));
}

Point2f utils::GetPointInBoundedRadius(const Circlef& circle, const Rectp& bounds, float shapeHalfWidth, float shapeHalfHeight)
{
	float safetyOffset{ 0 };
	float randAngle{ GetRand(2 * utils::g_Pi) };
	float randDist { GetRand(circle.radius) };
	float newX{ circle.center.x + randDist * float(cos(randAngle)) };
	float newY{ circle.center.y + randDist * float(sin(randAngle)) };

	if (bounds != Rectp{})
	{
		if (newX < bounds.center.x - bounds.halfWidth + shapeHalfWidth + safetyOffset)
			newX = bounds.center.x - bounds.halfWidth + shapeHalfWidth + safetyOffset;
		else if (newX > bounds.center.x + bounds.halfWidth - shapeHalfWidth - safetyOffset)
			newX = bounds.center.x + bounds.halfWidth - shapeHalfWidth - safetyOffset;

		if (newY < bounds.center.y - bounds.halfHeight + shapeHalfHeight + safetyOffset)
			newY = bounds.center.y - bounds.halfHeight + shapeHalfHeight + safetyOffset;
		else if (newY > bounds.center.y + bounds.halfHeight - shapeHalfHeight - safetyOffset)
			newY = bounds.center.y + bounds.halfHeight - shapeHalfHeight - safetyOffset;
	}
...
std::string utils::GetXmlFirstElement(std::ifstream& iS, const std::string& query, const std::string& attribute, const std::string& attrValue, const std::string& parentElement)
{
	std::vector<std::string> elements{ GetXmlElements(iS, query, attribute, attrValue, parentElement) };
...
std::vector<std::string> utils::GetXmlElements(std::ifstream& iS, const std::string& query, const std::string& attribute, const std::string& attrValue, const std::string& parentElement)
{
	std::vector<std::string> elements{};
	std::string line{};
	size_t splitPos{ query.find_first_of('/') };
	std::string currentElement{ query.substr(0, splitPos) };
	bool isParent{ splitPos != std::string::npos };
	bool done{ }, ignore{ };

	if (currentElement.empty() || iS.eof())
		return elements;
	...
	do
	{
		...
		ignore = false;
		std::getline(iS, line);
		if (line.find("</" + parentElement + ">") != std::string::npos)
			return elements;

		if (line.find("<" + currentElement) != std::string::npos)
		{
			if (attribute != "" && attrValue != "")
			{
				ignore = GetXmlAttributeValue(attribute, line) != attrValue;
			}
			...
			if (!isParent)
			{
				...
				if (attribute != "" && GetXmlAttributeValue(attribute, element) != attrValue)
					continue;
			...
			{
				return GetXmlElements(iS, query.substr(splitPos + 1), attribute, attrValue, currentElement);
			}
		}
...
std::string utils::GetXmlAttributeValue(const std::string& attrName, const std::string& element)
{
	size_t attrIdx{ element.find(attrName + "=") };
	size_t quote1Idx{ element.find(R"(")", attrIdx) };
	size_t quote2Idx{ element.find(R"(")", ++quote1Idx) };

	return element.substr(quote1Idx, quote2Idx - quote1Idx);
}

ElementalType utils::StrToElementalType(const std::string& str)
{
	if (str == "Fire")
...
Sprite* utils::GetElementalHitSprite(ElementalType type)
{
	int rowOffset{};
	switch (type)
	{
	...
	return new Sprite(ResourceManager::GetInstance()->GetSpriteSheet("Misc/Particules.png"), 0, 7, 1, 0, rowOffset, 0.05f);
}

Sprite* utils::GetMeleeHitSprite()
{
	return new Sprite(ResourceManager::GetInstance()->GetSpriteSheet("Misc/Particules.png"), 0, 7, 1, 0, 0, 0.05f);
}