...
namespace utils
{
	...
	const float g_Pi_2{ g_Pi / 2 };
	const float g_Pi_4{ g_Pi / 4 };
...
#pragma region OpenGLDrawFunctionality
	...
	void DrawRect(const Rectp& rect, float lineWidth = 1.0f);
	void FillRect(const Rectp& rect);
...
#pragma region CollisionFunctionality
	...
	bool IsPointInRect(const Point2f& p, const Rectp& r);
	...
	bool IsOverlapping(const Rectp& r1, const Rectp& r2);
	bool IsOverlapping(const Rectf & r1, const Rectf & r2);
	bool IsOverlapping(const Rectp& r, const Circlef& c);
	...
	bool IntersectRectLine(const Rectp& r, const Point2f& p1, const Point2f& p2, float& intersectMin, float& intersectMax);

#pragma endregion CollisionFunctionality

	float GetRand(float max, float min = 0.f);
	int GetRand(int max, int min = 0);
	int RoundToClosest(float number, int multiple = 1);
	float GetLength2(const Point2f& p0, const Point2f& p1);

	Point2f GetPointInBoundedRadius(const Circlef& circle, const Rectp& bounds = {}, float shapeHalfWidth = 0.f, float shapeHalfHeight = 0.f);

	std::string GetXmlFirstElement(std::ifstream& iS, const std::string& query, const std::string& attribute = "", const std::string& attrValue = "", const std::string& parentElement = "");
	std::vector<std::string> GetXmlElements(std::ifstream& iS, const std::string& query, const std::string& attribute = "", const std::string& attrValue = "", const std::string& parentElement = "");
	std::string GetXmlAttributeValue(const std::string& attrName, const std::string& element);

	ElementalType StrToElementalType(const std::string& str);
	Sprite* GetElementalHitSprite(ElementalType type);
	Sprite* GetMeleeHitSprite();
}