...
{
	Rectp();
	explicit Rectp(float centerX, float centerY, float halfWidth, float halfHeight);
	explicit Rectp(const Point2f& center, float halfWidth, float halfHeight);

	bool operator ==(const Rectp& other) const;
	bool operator !=(const Rectp& other) const;
